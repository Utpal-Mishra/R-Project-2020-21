---
title: "R Final Project : Breast Cancer Classification :: Notebook 1.1"
...
for(i in c(1:3)){
  outliers <- boxplot(data$area..nucA., plot=FALSE)$out #fetching out the ouliers from the boxplot
  x <- data #making a dimmy dataset
  x <- x[-which(x$area..nucA. %in% outliers), ] #remove ouliers from the data
  #boxplot(x[c(-1)], col = "red")
  data <- x #update original data without outliers
  
  outliers <- boxplot(data$area..nucB., plot=FALSE)$out
  x <- data #making a dimmy dataset
  x <- x[-which(x$area..nucB. %in% outliers), ] #remove ouliers from the data
  #boxplot(x[c(-1)], col = "red")
  data <- x #update original data without outliers
  
  outliers <- boxplot(data$area..nucC., plot=FALSE)$out
  x <- data #making a dimmy dataset
  x <- x[-which(x$area..nucC. %in% outliers), ] #remove ouliers from the data
  #boxplot(x[c(-1)], col = "red")
  data <- x #update original data without outliers
}

boxplot(data[c(-1)], col = "red", main = "Finding Outliers", notch = TRUE, xlab = "Features", ylab = "Statistically Calulated Values") #using boxplot to witness the data without outliers
```
...
```{r}
confusionMatrix(table(round((ifelse(dtresult %in% c("B", "B"), 0, 1) + ifelse(rfresult %in% c("B", "B"), 0, 1))/2), test$diagnosis..M.malignant..B.benign.)) #averaged ensemble model with maximum accuracy 96.32
```
...
confusionMatrix(table(round((ifelse(dtresult %in% c("B", "B"), 0, 1) + 
                             ifelse(svmresult %in% c("B", "B"), 0, 1))/2), test$diagnosis..M.malignant..B.benign.)) #averaged ensemble model with maximum accuracy 96.32
```
...
```{r}
confusionMatrix(table(round((ifelse(rfresult %in% c("B", "B"), 0, 1) + 
                             ifelse(nbresult %in% c("B", "B"), 0, 1))/2), test$diagnosis..M.malignant..B.benign.)) #averaged ensemble model with maximum accuracy 94.12
```
...
confusionMatrix(table(round((ifelse(rfresult %in% c("B", "B"), 0, 1)*1.00 +
                            (ifelse(knnmodel %in% c("B", "B"), 0, 1)*1.00))/2), test$diagnosis..M.malignant..B.benign.)) #averaged ensemble model with maximum accuracy 97.79
```
...
confusionMatrix(table(round((ifelse(rfresult %in% c("B", "B"), 0, 1)*0.90 + 
                            (ifelse(nnresult %in% c("B", "B"), 0, 1)*0.90))/2), test$diagnosis..M.malignant..B.benign.)) #averaged ensemble model with maximum accuracy 97.79
```
...
```{r}
confusionMatrix(table(round((ifelse(dtresult %in% c("B", "B"), 0, 1) + 
                             ifelse(nbresult %in% c("B", "B"), 0, 1))/2), test$diagnosis..M.malignant..B.benign.)) #averaged ensemble model with maximum accuracy 92.65
```
...
confusionMatrix(table(round((ifelse(svmresult %in% c("B", "B"), 0, 1)*0.80 +
                            (ifelse(knnmodel %in% c("B", "B"), 0, 1)*0.90))/2), test$diagnosis..M.malignant..B.benign.)) #averaged ensemble model with maximum accuracy 95.59
```
...
confusionMatrix(table(round((ifelse(svmresult %in% c("B", "B"), 0, 1) + 
                             ifelse(nnresult %in% c("B", "B"), 0, 1))/2), test$diagnosis..M.malignant..B.benign.)) #averaged ensemble model with maximum accuracy 95.59
```
...
confusionMatrix(table(round((ifelse(nbresult %in% c("B", "B"), 0, 1) + 
                             ifelse(nnresult %in% c("B", "B"), 0, 1))/2), test$diagnosis..M.malignant..B.benign.)) #averaged ensemble model with maximum accuracy 90.44
```
...
confusionMatrix(table(round((ifelse(rfresult %in% c("B", "B"), 0, 1)*0.90 + 
                            ...
                            (ifelse(nnresult %in% c("B", "B"), 0, 1)*0.90))/3), test$diagnosis..M.malignant..B.benign.)) #averaged ensemble model with maximum accuracy 97.06
```
...
confusionMatrix(table(round((ifelse(rfresult %in% c("B", "B"), 0, 1)*0.90+
                            ...
                            (ifelse(knnmodel %in% c("B", "B"), 0, 1)*0.90))/3), test$diagnosis..M.malignant..B.benign.)) #averaged ensemble model with maximum accuracy 97.06
```
