---
title: "R Final Project : Breast Cancer Classification :: Notebook 2"
...
```{r}
boxplot(data[c(-1)], col = "red", main = "Data with Outliers", notch = TRUE, xlab = "Features", ylab = "Statistically Calulated Values")  #using boxplot to find the outliers
```
...
boxplot(data[c(-1)], col = "red", main = "Data without Outliers", notch = TRUE, xlab = "Features", ylab = "Statistically Calulated Values") #using boxplot to witness the data without outliers
```
...
library(Boruta) #using boruta to find the optimla features within the dataset
...
confusionMatrix(table(round((ifelse(dtresult %in% c("B", "B"), 0, 1) + 
                             ifelse(rfresult %in% c("B", "B"), 0, 1))/2), test$diagnosis..M.malignant..B.benign.)) #the maximum accuracy of the model is 92.50
```
...
```{r}
confusionMatrix(table(round((ifelse(rfresult %in% c("B", "B"), 0, 1) + 
                             ifelse(nbresult %in% c("B", "B"), 0, 1))/2), test$diagnosis..M.malignant..B.benign.)) #the maximum accuracy of the model is 95.00
```
...
confusionMatrix(table(round((ifelse(rfresult %in% c("B", "B"), 0, 1)*0.90 + 
                            (ifelse(nnresult %in% c("B", "B"), 0, 1)*0.90))/2), test$diagnosis..M.malignant..B.benign.)) #the maximum accuracy of the model is 95.00
```
...
confusionMatrix(table(round((ifelse(svmresult %in% c("B", "B"), 0, 1) + 
                             ifelse(nnresult %in% c("B", "B"), 0, 1))/2), test$diagnosis..M.malignant..B.benign.)) #the maximum accuracy of the model is 97.50
```
...
confusionMatrix(table(round((ifelse(nbresult %in% c("B", "B"), 0, 1) + 
                             ifelse(nnresult %in% c("B", "B"), 0, 1))/2), test$diagnosis..M.malignant..B.benign.)) #the maximum accuracy of the model is 98.75
```
...
confusionMatrix(table(round((ifelse(rfresult %in% c("B", "B"), 0, 1)*0.90 + 
                            ...
                            (ifelse(nnresult %in% c("B", "B"), 0, 1)*0.90))/3), test$diagnosis..M.malignant..B.benign.)) #the maximum accuracy of the model is 97.50
```
